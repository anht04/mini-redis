...
namespace MiniRedis.Data
{
    public class RedisDatabase
    {
        ...
        public string XRange(RedisEntry cacheKey, string startId, string endId, XRangeCommandPurpose purpose)
        {
            ...
            if ((purpose == XRangeCommandPurpose.NormalQuery && !RedisStreamDataId.TryParseXRangeStreamDataId(startId, out startTimestamp, out startSequenceNumber, out startPattern)) ||
                !RedisStreamDataId.TryParseXRangeStreamDataId(endId, out var endTimestamp, out var endSequenceNumber, out var endPattern))
            {
                throw new InvalidOperationException(RedisErrorMessages.XAddStreamDataIdInvalidFormat);
            }

            if ((purpose == XRangeCommandPurpose.NormalQuery && startPattern != StreamDataIdPattern.FullForm && startPattern != StreamDataIdPattern.AutoGeneratedSequence) ||
            ...
            if (purpose == XRangeCommandPurpose.NormalQuery)
            {
                parsedStartId = RedisStreamDataId.Create(startTimestamp!.Value, startSequenceNumber.Value);
            }
            var parsedEndId =
                RedisStreamDataId.Create(endTimestamp!.Value, endSequenceNumber!.Value);
