...
namespace MiniRedis.Data
{
    public class RedisDatabase
    {
        ...
        public string XRead(RedisEntry cacheKey, string startId)
        {
            if (!_cache.TryGetValue(cacheKey, out var value))
            {
                return RESPFormatHelper.FormatArray((string?)null);
            ...
            if (!RedisStreamDataId.TryParseXRangeStreamDataId(startId, out var startTimestamp, out var startSequenceNumber, out var startPattern) || 
                startPattern != StreamDataIdPattern.FullForm && startPattern != StreamDataIdPattern.AutoGeneratedSequence)
            {
                throw new InvalidOperationException(RedisErrorMessages.XAddStreamDataIdInvalidFormat);
            }
            ...
            var parsedStartId = RedisStreamDataId.Create(startTimestamp!.Value, startSequenceNumber.Value);
            
            var result = streamData
                .GetRangeGreaterThan(parsedStartId)
                .Select(kp =>
                    new KeyValuePair<string, List<string>>(
                        kp.Key.ToString(),
                        kp.Value.SelectMany(v => v.ToKeyValueStringArray()).ToList()))
            ...
            return RESPFormatHelper.FormatArray(cacheKey.Key, result);
        }
