...
namespace MiniRedis.Data
{
    public class RedisDatabase
    {
        ...
        public string XRange(RedisEntry cacheKey, string startId, string endId)
        {
            ...
            if (!RedisStreamDataId.TryParseXRangeStreamDataId(startId, out var startTimestamp, out var startSequenceNumber,
                    out var startPattern) ||
                !RedisStreamDataId.TryParseXRangeStreamDataId(endId, out var endTimestamp, out var endSequenceNumber,
                    out var endPattern))
            {
                throw new InvalidOperationException(RedisErrorMessages.XAddStreamDataIdInvalidFormat);
            }

            if ((startPattern != StreamDataIdPattern.FullForm && startPattern != StreamDataIdPattern.AutoGeneratedSequence) ||
                (endPattern != StreamDataIdPattern.FullForm && endPattern != StreamDataIdPattern.AutoGeneratedSequence))
            {
                throw new InvalidOperationException(RedisErrorMessages.XAddStreamDataIdInvalidFormat);
            }

            RedisStreamData streamData = value.AsStream();
            ...
            var parsedStartId =
                RedisStreamDataId.Create(startTimestamp!.Value, startSequenceNumber.Value, StreamDataIdPattern.FullForm);
            var parsedEndId =
                RedisStreamDataId.Create(endTimestamp!.Value, endSequenceNumber!.Value, StreamDataIdPattern.FullForm);
        ...
        {
            var newStreamData = CreateNewStreamData(streamId.Key, dataId, streamDataValues, out var idGenerationBehaviour);
            var newDataId = newStreamData.GetCurrentSmallestId()!;

            var cacheValue = new RedisValue(newStreamData);

            if (!_cache.TryGetValue(streamId, out var streamData))
            {
                _cache.Add(streamId, cacheValue);
                return newDataId;
            }
        ...
        private RedisStreamData CreateNewStreamData(string streamId, string dataId, List<RedisStreamDataValue> streamDataValues,
            out StreamDataIdPattern idPattern)
        {
            var baseNewDataId = GenerateNewDataId(streamId, dataId, out idPattern);
            return RedisStreamData.Create(baseNewDataId, streamDataValues, idPattern);
        }
